...
begin
	...
	for i in CartesianIndices(titles)
		Label(fig[i[1], i[2]][0,1:2], titles[i], font=:bold, fontsize=20,)
	end
	...
	for (svr_region, trendy_region) in zip(svr_data_dfs, trendy_data_dfs)
		...
		band!(axs[axis_idx], years, trendy_mean - trendy_sd, trendy_mean + trendy_sd, color=(:lightgray, 1),label="Trendy SD")
		...
		for model in models
			lines!(axs[axis_idx], years, trendy_data_dfs[trendy_region[1]][:, model], color=(:gray,0.7), linewidth=0.8, label="Trendy")
