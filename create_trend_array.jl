...
# ╔═╡ 193c368e-0dd3-11ef-3424-6b9e399d5929
begin
	using Printf
	using CairoMakie
	using MLJ
...
# ╔═╡ 7c46fffe-f36a-4e0b-ba8e-4ffd04900964
begin
	function create_all_versions()
		...
		for (iv, version) in enumerate(versions)
			...
			for (i, year) in enumerate(years)
				if year in 2016:2020 && version == "005"
					...
					continue
				elseif year in 2019:2020 && version == "006"
					all_months[:,:,:,i] .= missing
					continue
				end
				year = string(year)
				filename = @sprintf("GPP.ALL.AVERAGE.%s.bsq.flt", year)
				filepath = @sprintf("./out_c%s/MONTH_cor/%s", version[2:3], filename)
				println(filepath)
				
				data = Array{Float32}(undef, (1440, 720, 12))
				data = Array{Float32}(undef, (480, 360, 12))
				read!(filepath, data)
		...
		replace!(all_versions, -990 => missing);
		return all_versions
	end
end

# ╔═╡ e5c9f17a-77fd-4f4b-bf5e-fbe2e8a7a5e0
begin
...
# ╔═╡ 16852beb-9bf1-47f0-802a-cd7d8cc94141
begin
...
# ╔═╡ e51f7a23-5d25-4eeb-8ffa-94b2da199a32
begin
	all_versions = create_all_versions()
	yearly_aves = create_yearly_averages(all_versions)
	trend_array = create_trend_array(yearly_aves, ts_regr)
end

# ╔═╡ 98fa9c2d-7e01-4248-91d8-371657becfc7
...
manifest_format = "2.0"
project_hash = "b321f3e399f9159225420e440aa7cfcf90f91bc1"
...
git-tree-sha1 = "2387719966533d2cf996a0b75a227204008d3948"
