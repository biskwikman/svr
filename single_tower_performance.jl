...
# ╔═╡ 98ceaa0e-489e-11ef-3c5a-c95370e6ceae
begin
	using DataFrames
	using CSV
	using Printf
...
ec_df[ec_df.site_code .== "RUFyo", :]

# ╔═╡ 86cec5e0-e9ec-4a00-8e02-70c0fe632294
tower_gpp_df = select(filter(row -> (row.site_code == "RUFyo" && row.year in row.syear:row.eyear), ec_df), [:year, :doy] => ByRow((y,d) -> "$(y)_$(d)_10") => :Key, :gpp => :GPP_Obs)
...
begin
	...
	for ens in ensembles
		output_file = @sprintf("./CROSSVAL_c61/output_lvmar/CV_%i_ALL_GPP.csv", ens)
		version_df = DataFrame(CSV.File(output_file, header=false))
		select!(version_df, Between("Column1", "Column6"))
		insertcols!(version_df, 1, :Version => :c61)
	...
	# filter(row -> row.Site_ID == 10, output_df)
	output_gpp_df = combine(groupby(filter(row -> row.Site_ID == 10, output_df), :Key), :GPP => mean => :GPP_Output)
...
begin
	f = Figure()
	ax = Axis(f[1,1], xticks=1:46:length(gpp_df[:, :Key]), xtickformat= (values) -> ["$(i+1999)" for (i,value) in enumerate(values)])
	lines!(ax, 1:length(gpp_df[:, :Key]), gpp_df[:, :GPP_Obs])
	lines!(ax, 1:length(gpp_df[:, :Key]), gpp_df[:, :GPP_Output], color=:red)
	f
end

# ╔═╡ 4c566a07-a210-425f-8cd4-792931f67a46
for i in 1:2:10
	println(i)
end
...
manifest_format = "2.0"
project_hash = "bc124abf3db2e800c862ca2880996dabb239f3dc"
